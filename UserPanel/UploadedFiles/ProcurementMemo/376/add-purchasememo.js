...
const AddPurchaseMemo = () => {
    ...
    const handleSubmit = async (values, isSubmitted) => {
        ...
        const detailsCount = values.items?.filter(row =>
            row.itemGroupId || row.itemId || row.departmentId || row.qty || row.deliveryDate
        ).length || 0;
        ...
        if (!hasRemarks && detailsCount === 0) {
            Swal.fire({
                icon: 'error',
                title: 'Missing Data',
                text: 'Please enter remarks or add at least one detail row.'
            });
            return;
        }
                                                ...
                                                <div className="col-xl-12 mt-lg-4">
                                                    <Col md="12">
                                                        <FormGroup>
                                                            ...
                                                            <Field as="textarea"     checked={values.isNew=== 1} name="remarks" rows={3} className="form-control" />
                                                        </FormGroup>
                                                    </Col>
                                                </div>
                                                <div className="col-xl-12 mt-lg-4">
