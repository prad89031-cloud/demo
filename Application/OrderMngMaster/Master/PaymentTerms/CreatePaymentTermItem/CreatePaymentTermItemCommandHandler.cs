...
{
    public class CreatePaymentTermItemCommandHandler : IRequestHandler<CreatePaymentTermItemCommand, object>
    {

        private readonly IPaymentTermRepository _repository;
        private readonly IUnitOfWorkDB1 _unitOfWork;
        
        public CreatePaymentTermItemCommandHandler(IPaymentTermRepository repository, IUnitOfWorkDB1 unitOfWork)
        {
            _repository = repository;
            _unitOfWork = unitOfWork;
        }
        #region Handle
        public async Task<object> Handle(CreatePaymentTermItemCommand command, CancellationToken cancellationToken)
        {
            ...
            var data = await _repository.CreatePaymentTermAsync(payTerms);
            _unitOfWork.Commit();
            return data;
